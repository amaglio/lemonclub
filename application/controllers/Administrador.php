...
public function __construct()
{
	...
	$this->load->model('Ingrediente_model');
...
public function pedidos($vista='tabla') // Index pedidos 
{
...
public function buscar_pedidos($vista=null) // Buscar pedidos 
{
	$data['mensaje'] = $this->session->flashdata('mensaje');
	$output = (object)array('output' => '' , 'js_files' => array() , 'css_files' => array());
	$output->titulo = traer_titulo($this->uri->segment(2));
...
public function productos()	   // Index de productos 
{
...
public function producto_dia() // Producto del dia 
{
	$crud = new grocery_CRUD();
	$crud->set_theme('datatables');
	$crud->set_language("spanish");
...
public function set_producto_grupo_ingrediente() // Configurar ingrediente de un grupo de un producto 
{
...
public function delete_producto($primary_key) // Eliminar el producto
{
	$_POST['id_producto'] = $primary_key;
   	$this->form_validation->set_data($_POST);

	if ($this->form_validation->run('delete_producto') == FALSE):
		...
		$query = $this->Producto_model->abm_producto( 'B', $this->input->post() );

		if( $query ):

 			$this->session->set_flashdata('mensaje', 'Se le ha eliminado el tipo de producto exitosamente ');
			$return["error"] = FALSE;

		else:

			$this->session->set_flashdata('mensaje', 'Error, no se le ha eliminado el tipo de producto.');
			$return["error"] = TRUE;
...
public function agregar_grupo_producto() // Agregar un grupo a un producto
{
	...
	if ($this->form_validation->run('agregar_grupo_producto') == FALSE):
		...
		$query = $this->Producto_model->abm_grupo_producto( 'A', $this->input->post() );
...
public function eliminar_grupo_producto() // Eliminar un grupo a un producto
{
	...
	if ($this->form_validation->run('eliminar_grupo_producto') == FALSE):
		...
		$query = $this->Producto_model->abm_grupo_producto( 'B', $this->input->post() );
...
public function grupo_ingredientes() // Index de grupo_ingredientes 
{
	$crud = new grocery_CRUD();
	$crud->set_theme('datatables');
	$crud->set_table('grupo');
	$crud->where('grupo.fecha_baja IS NULL');
    ...
    $crud->callback_delete(array($this,'delete_grupo_ingrediente'));
...
public function delete_grupo_ingrediente($primary_key) // Eliminar el grupo de ingrediente
{	
	chrome_log("delete_grupo_ingrediente");

	$_POST['id_grupo'] = $primary_key;
   	$this->form_validation->set_data($_POST);

	if ($this->form_validation->run('delete_grupo_ingrediente') == FALSE):
		...
		$query = $this->Grupo_model->abm_grupo( 'B', $this->input->post() );

		if( $query ):

 			$this->session->set_flashdata('mensaje', 'Se le ha eliminado el grupo exitosamente ');
			$return["error"] = FALSE;

		else:

			$this->session->set_flashdata('mensaje', 'Error, no se le ha eliminado el grupo.');
			$return["error"] = TRUE;
...
public function ver_grupos_producto() // Index de grupos de un producto
{
...
public function ver_agregar_ingrediente_grupo() // Ver agregar ingrediente a un grupo
{
 	$datos['mensaje'] = $this->session->flashdata('mensaje');
	$output = (object)array('output' => '' , 'js_files' => array() , 'css_files' => array());
	$output->titulo = traer_titulo($this->uri->segment(2));
...
public function agregar_ingrediente_grupo() // Procesar agregar ingrediente a un grupo
{
	...
	if ($this->form_validation->run('agregar_ingrediente_grupo') == FALSE):
		...
		$query = $this->Grupo_model->abm_ingrediente_grupo( 'A', $this->input->post() ); 
...
public function delete_ingrediente_grupo() // Procesar eliminar ingrediente a un grupo
{	
	if ($this->form_validation->run('delete_ingrediente_grupo') == FALSE):
		...
		$query = $this->Grupo_model->abm_ingrediente_grupo( 'B', $this->input->post() );
...
public function ajax_grupo() // Buscar grupo para agregar a un producto
{
	...
	{	
		$query = $this->Grupo_model->get_grupos_producto_by_string($buscar,$id_producto );
...
public function ingredientes() // Index de ingregientes 
{
	$crud = new grocery_CRUD();
...
{
	$_POST['id_ingrediente'] = $primary_key;
   	$this->form_validation->set_data($_POST);

	if ($this->form_validation->run('delete_ingrediente') == FALSE):
		...
		$query = $this->Ingrediente_model->abm_ingrediente( 'B', $this->input->post() );
...
public function ajax_ingrediente() // Buscar ingredientes para agregar a un grupo
{
	...
	if( isset($buscar) && strlen($buscar) > 1 )
	{	
		$query = $this->Ingrediente_model->get_ingrediente_grupo_by_string($buscar,$id_grupo );
...
public function tipos_productos() // Index de tipos productos 
{
	$crud = new grocery_CRUD();
	$crud->set_theme('datatables');
	...
	$crud->unset_read();
	$crud->callback_delete(array($this,'delete_producto_tipo'));
...
public function delete_producto_tipo($primary_key)
{	
	$_POST['id_producto_tipo'] = $primary_key;
   	$this->form_validation->set_data($_POST);

	if ($this->form_validation->run('delete_producto_tipo') == FALSE):
		...
		$query = $this->Producto_tipo_model->abm_producto_tipo( 'B', $this->input->post() );
...
public function usuarios_invitados() // Index de usuarios
{
	$crud = new grocery_CRUD();
	$crud->set_theme('datatables');
...
public function usuarios_registrados() // Index registrados
{
	$crud = new grocery_CRUD();
	$crud->set_theme('datatables');
	$crud->set_language("spanish");
...
public function tipos_ingredientes() // Index tipos de ingredientes
{
	$crud = new grocery_CRUD();
	$crud->set_theme('datatables');
...
public function estadisticas()  // Index de estadisticas
{
	$data['mensaje'] = $this->session->flashdata('mensaje');
	$output = (object)array('output' => '' , 'js_files' => array() , 'css_files' => array());
	$output->titulo = traer_titulo($this->uri->segment(2));
...
public function buscar_estaditicas() // Buscar estadisticas
{	
...
public function _callback_webpage_url($value, $row) // Muestra imagen 
{
...
public function imprimir_comanda($id_pedido) // Imprimir comandas
{
